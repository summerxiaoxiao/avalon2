<template>
  <div class="h-box-column h-map-box" >
    <div class="d-flex h-map_tip" >
      <div class="h-map_tip__bg"></div>
      <div class="d-flex flex-column h-map_tip__label">
        <div class="flex-row h-map__label__title">
          <span>{{title}}</span>
          <span class="selector">
            <select class="">
            <option value="冀北公司">冀北公司</option>
              <option value="冀北公司">冀北公司2</option>
          </select>
            <i class="select-down iconfont  icon-xiala"></i>
          </span>
        </div>
    ...
    <div class="h-map-box_area h-box-row h-flex-auto" >
      ...
      <div class="h-map_marker h-map_marker__arrow" v-show="showTip">
        <div class="h-map_marker__bg"></div>
        <div class="h-map_marker__label">
          <div class="h-map_marker__label__title">山东省</div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...Vuex.mapState({
        'geoCoordMapData': state => state.qmys.geoCoordMapData,
        'mapFeaturesData': state => state.qmys.mapFeaturesData,
        'registerMapData': state => state.qmys.registerMapData,
        'localType': state => state.qmys.localType
      })
    },
    data () {
      return {
    ...
    methods: {
      load () {
        this.$store.dispatch('qmys/loadMap', {mapType: this.mapType, mapMark: this.mapMark}).then(() => {
          // 注册map
    ...
    .h-map-box_area {
      ...
      .h-map_marker__arrow {
        &::before {
          ...
          border-right: 10px solid #6484b2;
          left: 1px;
          z-index: 99999;
        }
        &::after {
