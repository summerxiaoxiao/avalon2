<template>
  <div class="h-treetable__main" ref="treetable_main" :class="mainClasses">
    <div class="h-treetable__head" >
      <table ref="treetable2" :class="classes" >
        <thead class="h-treetable__thead" >
        <tr>
          <th v-if="selection" class="h-treetable__selection">
            <input type="checkbox" class="selection" value="" @change="(evt)=>{ handleChange(evt, '') }" />
          </th>
          <template v-for="column in columns">
            <th v-if="column.width" :key="column.key"
                :style=headerStyle(column)>{{column.label}}</th>
            <th v-else :key="column.key"
                :style=headerStyle(column)>{{column.label}}</th>
          </template>
        </tr>
        </thead>
    ...
    <div class="h-treetable__content" ref="treetable_content">
      ...
      <template v-for="(row, index) in rows" >
        <tr :key="row[columnKey]" class="h-treetable__row "
          ...
          <template v-for="column in columns">
            ...
            <td v-else :style=columnStyle(column)
                :key="row[columnKey] + column.key"
                :data-key="column.key"
                :data-raw="row[column.key]" >{{row[column.key]}}</td>
          </template>
          </tr>
      </template>
      </tbody>
...
const props = {
  ...
  display: {
    type: String,
    default: 'expand'  // expand, collapsed, none
  },
  // 无边框形式
  borderless: {
    type: Boolean,
...
export default {
  ...
  mounted () {
    ...
    if (this.scroll) {
      var thead = this.$refs.treetable.querySelector('thead')
      var html = thead.innerHTML
      this.$refs.treetable2.querySelector('thead').innerHTML = html
  ...
  methods: {
    ...
    headerStyle (column) {
      ...
      if (column.columnStyle) {
        style = JSON.parse(JSON.stringify(column.columnStyle))
      }
      if (column.align) {
