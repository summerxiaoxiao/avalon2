 ...
 * @events  @on-row-click 单击行或者单击操作按钮方法
   ...
   @props   items 显示的结构化数据
            ...
            options: {
              ...
              isExpandClick: false 点击节点不展开
            }
 -->
<template>
  <div :style="{width:tableWidth}" class='autoTbale'>
    <div class="table-header">
      <table class="table table-bordered" id='hl-tree-table-header'>
        <thead>
        </thead>
      </table>
    </div>
          ...
          <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
            ...
            <div v-if="column.type === 'action'">
              ...
              <label @click="toggle(index,item)" v-if="!column.type">
               ...
               <!-- <template v-for="(field,f) in options.bgcolorField" v-if="f===snum">
        ...
        if (this.options.expandAll) {
          this.initItems.forEach((item, index) => {
            ...
            item.children && item.children.forEach((child, childIndex) => {
              ...
              this.$set(this.initItems[index + childIndex + 1], 'level', item.level + 1)
              this.$set(this.initItems[index + childIndex + 1], 'spaceHtml', spaceHtml)
              this.$set(this.initItems[index + childIndex + 1], 'isShow', true)
              this.$set(this.initItems[index + childIndex + 1], 'expanded', true)
            })
          })
        }
